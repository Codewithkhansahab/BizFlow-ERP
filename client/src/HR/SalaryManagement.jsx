...
export default function SalaryManagement() {
  ...
  const loadEmployees = async () => {
    try {
      const { data } = await axios.get(`${backendUrl}/api/employee/employees`, {
  ...
  const loadSalaries = async (opts = {}) => {
    ...
    Object.keys(q).forEach(
      (k) => (q[k] === "" || q[k] === undefined) && delete q[k]
    );
    try {
      setLoading(true);
  ...
  useEffect(() => {
    loadSalaries(); /* eslint-disable-next-line */
  }, [page, limit]);
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await axios.delete(`${backendUrl}/api/salaries/${id}`, {
        withCredentials: true,
      });
  ...
  const openPay = async (item) => {
    ...
    const employeeName = item.employee?.user?.name || "Unknown Employee";
    const result = await showPaymentMethodDialog(employeeName);

    if (result.isConfirmed) {
      const paymentMethod = result.value;
      await markAsPaid(item._id, paymentMethod);
    }
  };

  const markAsPaid = async (salaryId, paymentMethod) => {
    try {
      await axios.patch(
        `${backendUrl}/api/salaries/${salaryId}/pay`,
        { paymentMethod },
        { withCredentials: true }
      );
    ...
    <DashboardLayout roleTitle="HR">
      <Row className="align-items-end g-3">
        ...
        <Col md={3} className="d-flex gap-2">
          ...
          <Button variant="success" className="ms-auto" onClick={openCreate}>
            + Create
          </Button>
        </Col>
      </Row>
                  ...
                  <tr key={item._id}>
                    ...
                    <td className="d-flex gap-2">
                      ...
                      <Button
                        size="sm"
                        variant={
                          item.status === "Paid" ? "success" : "outline-success"
                        }
