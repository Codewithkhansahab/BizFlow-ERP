...
export const approveUserRegistration = async (req, res) => {
  try {
    ...
    if (
      ...
      user.approvalRequiredRole !== approver.role
    ) {
      return res
        .status(400)
        .json({ message: "This request is not pending your approval" });
    }
    ...
    try {
      const { subject, html, text } = buildApprovalDecisionEmail({
        ...
        decision: "Approved",
        frontendUrl:
          process.env.FRONTEND_URL || "https://bizflow-erp-1.onrender.com",
  ...
  try {
    ...
    try {
      await Notification.create({
        ...
        message: reason
          ? `Your ${user.role} account was rejected. Reason: ${reason}`
          : `Your ${user.role} account was rejected.`,
        type: "General",
        category: "General",
      });
    } catch (notifyErr) {
    ...
    if (role === "Admin") {
      ...
      if (adminExists) {
        return res.status(400).json({
          message:
            "An admin already exists in the system. Please contact the system administrator.",
  ...
  try {
    ...
    if (user.approvalStatus === "Pending") {
      return res.status(403).json({
        message: `Account pending ${user.approvalRequiredRole} approval`,
      });
    }
    if (user.approvalStatus === "Rejected") {
      return res.status(403).json({
        message: "Your registration was rejected. Please contact support/HR.",
      });
    }
...
export const updateProfile = async (req, res) => {
  try {
    ...
    const anyPasswordFieldProvided = Boolean(
